...
export function SpotlightDust() {
    ...
    useEffect(() => {
        ...
        // Start the animation loop after the main thread is idle to avoid adding
        // to the initial long-task burst (reduces TBT / "Avoid long main-thread tasks").
        const startLoop = () => {
            rafRef.current = requestAnimationFrame(draw);
        };
        if (typeof (window as Window & { requestIdleCallback?: (cb: () => void, opts?: { timeout: number }) => void }).requestIdleCallback === 'function') {
            (window as Window & { requestIdleCallback: (cb: () => void, opts?: { timeout: number }) => void }).requestIdleCallback(startLoop, { timeout: 300 });
        } else {
